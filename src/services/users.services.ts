...
class userService {
  ...
  private signEmailVerifyToken(user_id: string) {
    return signJwt({
      ...
      options: { algorithm: 'HS256', expiresIn: process.env.EMAIL_VERIFY_TOKEN_EXPIRES_IN }
    })
  }
  constructor() {}
  ...
  async register(payload: { email: string; password: string; name: string }) {
    ...
    await Promise.all([
      ...
      databaseServices.refreshTokens.insertOne(
        new RefreshTokenSchema({ user_id: new ObjectId(user_id.toString()), token: refreshToken as string })
      )
    ])
  ...
  async login(payload: { user_id: string }) {
    ...
    await databaseServices.refreshTokens.insertOne(
      new RefreshTokenSchema({ user_id: new ObjectId(payload.user_id), token: refreshToken as string })
    )
  ...
  logout(payload: { user_id: string }) {
    ...
    return new Promise((resolve, reject) => {
      databaseServices.refreshTokens
        ...
        .then((resultDelete) => {
          resultDelete.deletedCount === 0 ? reject('Token is removed') : resolve(true)
        })
        .catch((err) => {
          reject(err)
        })
